...
export async function POST(request: NextRequest) {
	try {
		...
		const endTime = new Date(
			appointmentDate.getTime() + validatedData.duration * 60000,
		);

		const conflicts = appointments.filter((apt) => {
			if (
				apt.practiceId !== user.id ||
				apt.dentistId !== validatedData.dentistId
			) {
				return false;
			}
			...
			const existingEnd = new Date(
				existingStart.getTime() + apt.duration * 60000,
			);
...
export async function PUT(request: NextRequest) {
	try {
		...
		if (validatedData.date || validatedData.duration) {
			...
			const newDuration =
				validatedData.duration || existingAppointment.duration;

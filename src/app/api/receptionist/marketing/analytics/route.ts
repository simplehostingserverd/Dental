...
export async function GET(request: NextRequest) {
	try {
		...
		const topPosts: TopPost[] = [
			{
				id: "post_1",
				content:
					"🦷 Did you know that regular dental cleanings can prevent gum disease? Book your appointment today!",
				platform: "Instagram",
			...
			},
			{
				id: "post_2",
				content:
					"✨ Transform your smile with our professional teeth whitening service!",
				platform: "Facebook",
			...
			},
			{
				id: "post_3",
				content:
					"Quick tip: Brush your teeth for at least 2 minutes, twice a day! ⏰🦷",
				platform: "Twitter",
		...
		if (platform) {
			filteredMetrics = socialMetrics.filter(
		...
		const avgEngagement =
			filteredMetrics.reduce((sum, m) => sum + m.engagement, 0) /
			filteredMetrics.length;
		const avgGrowth =
			filteredMetrics.reduce((sum, m) => sum + m.growth, 0) /
			filteredMetrics.length;

		// Calculate engagement trends
		const currentPeriodEngagement = engagementData
			.slice(-7)
			.reduce((sum, d) => sum + d.likes + d.comments + d.shares, 0);
		const previousPeriodEngagement = engagementData
			.slice(-14, -7)
			.reduce((sum, d) => sum + d.likes + d.comments + d.shares, 0);
				...
				insights: [
					{
						...
						description:
							"Instagram followers increased by 18.3% this month, driven by before/after content.",
					...
					},
					{
						...
						description:
							"Posts published on Friday at 6:00 PM receive 35% more engagement.",
					...
					},
					{
						...
						description:
							"Patient testimonial posts have the highest engagement rate at 9.1%.",

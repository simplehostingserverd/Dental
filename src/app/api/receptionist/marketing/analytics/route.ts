...
export async function GET(request: NextRequest) {
	try {
		...
		const analytics =
			mockAnalytics[user.id as keyof typeof mockAnalytics] ||
			mockAnalytics["practice-1"];
...
export async function POST(request: NextRequest) {
	try {
		...
		console.log(
			`Refreshing analytics for platforms: ${platforms?.join(", ") || "all"}`,
		);
		...
		const refreshedAnalytics = {
			...
			overview: {
				...analytics.overview,
				totalFollowers:
					analytics.overview.totalFollowers + Math.floor(Math.random() * 50),
				totalEngagement:
					analytics.overview.totalEngagement + Math.floor(Math.random() * 20),
			},
...
export async function PUT(request: NextRequest) {
	try {
		...
		const body = await request.json();
		const { autoRefresh, refreshInterval, enabledPlatforms, trackingMetrics } =
			body;

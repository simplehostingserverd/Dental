...
const TelegramMessageSchema = z.object({
  ...
  type: z.enum(['text', 'appointment_reminder', 'appointment_confirmation', 'payment_reminder', 'post_treatment', 'promotion']),
    ...
    switch (validatedData.type) {
      ...
      case 'appointment_reminder':
        if (!validatedData.data?.patientName || !validatedData.data?.appointmentDate || 
          ...
          return NextResponse.json(
            { error: "Patient name, appointment date, time, doctor name, and clinic address are required" },
            { status: 400 }
          );
        }
      ...
      case 'appointment_confirmation':
        if (!validatedData.data?.patientName || !validatedData.data?.appointmentDate || 
            !validatedData.data?.appointmentTime || !validatedData.data?.treatmentType) {
          return NextResponse.json(
            { error: "Patient name, appointment date, time, and treatment type are required" },
            { status: 400 }
          );
        }
      ...
      case 'payment_reminder':
        if (!validatedData.data?.patientName || !validatedData.data?.amount || 
            !validatedData.data?.dueDate || !validatedData.data?.invoiceNumber) {
          return NextResponse.json(
            { error: "Patient name, amount, due date, and invoice number are required" },
            { status: 400 }
          );
        }
      ...
      case 'promotion':
        if (!validatedData.data?.patientName || !validatedData.data?.promotionTitle || 
            !validatedData.data?.promotionDetails || !validatedData.data?.validUntil) {
          return NextResponse.json(
            { error: "Patient name, promotion title, details, and valid until date are required" },
            { status: 400 }
          );
        }

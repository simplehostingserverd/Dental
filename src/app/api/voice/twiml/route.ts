...
export async function POST(request: NextRequest) {
	try {
		...
		twiml.say('We apologize, but our receptionist is currently unavailable. Please leave a message after the tone.');
		twiml.record({
			timeout: 30,
			transcribe: true,
		...
		// Fallback TwiML
		const twiml = new VoiceResponse();
		twiml.say('We apologize, but we are experiencing technical difficulties. Please try calling again later.');

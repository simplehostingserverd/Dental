...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!firstName || !lastName || !practiceName || !workEmail || !phoneNumber || !practiceSize || !password) {
    ...
    const passwordStrength = PasswordService.calculatePasswordStrength(password)
    if (passwordStrength < 60) { // Require at least medium strength
      return NextResponse.json(

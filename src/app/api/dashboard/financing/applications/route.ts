...
export async function POST(request: NextRequest) {
	try {
		...
		if (!financingOption) {
			return NextResponse.json(
				{ error: "Financing option not found" },
				{ status: 404 },
			);
		}
		...
		if (
			validatedData.amount < financingOption.minAmount ||
			validatedData.amount > financingOption.maxAmount
		) {
			return NextResponse.json(
				{
		...
		const monthlyPayment =
			monthlyInterestRate > 0
				? (validatedData.amount *
						monthlyInterestRate *
						(1 + monthlyInterestRate) ** validatedData.termMonths) /
					((1 + monthlyInterestRate) ** validatedData.termMonths - 1)
			...
			try {
				...
				if (isApproved) {
					...
					const approvedAmount =
						Math.round(validatedData.amount * approvalPercentage * 100) / 100;

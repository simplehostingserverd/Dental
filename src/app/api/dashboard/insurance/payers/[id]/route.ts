...
export async function PUT(
	...
	try {
		...
		if (
			validatedData.payerCode &&
			validatedData.payerCode !== existingPayer.payerCode
		) {
...
export async function DELETE(
	...
	try {
		...
		if (
			existingPayer._count.claims > 0 ||
			existingPayer._count.patientInsurances > 0
		) {
			return NextResponse.json(
				{
					error:
						"Cannot delete payer with associated claims or patient insurances. Deactivate instead.",
				},
				{ status: 400 },
			);
		}

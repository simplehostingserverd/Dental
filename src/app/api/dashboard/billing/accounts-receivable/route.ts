...
const updateARSchema = z.object({
  collectionStatus: z.enum(["CURRENT", "PAST_DUE", "COLLECTIONS", "BAD_DEBT", "WRITTEN_OFF"]).optional(),
  ...
  try {
    ...
    for (const patient of patients) {
      ...
      for (const entry of patient.ledgerEntries) {
        const daysDiff = Math.floor((today.getTime() - entry.transactionDate.getTime()) / (1000 * 60 * 60 * 24));
        const amount = entry.amount;

        if (amount > 0) { // Only positive amounts (charges)
          if (daysDiff <= 30) {
      ...
      const totalBalance = currentBalance + days30Balance + days60Balance + days90Balance + days120PlusBalance;

      // Determine collection status
      let collectionStatus: "CURRENT" | "PAST_DUE" | "COLLECTIONS" | "BAD_DEBT" | "WRITTEN_OFF" = "CURRENT";

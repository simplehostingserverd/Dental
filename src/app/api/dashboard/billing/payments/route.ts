...
const recordPaymentSchema = z.object({
  ...
  paymentMethod: z.enum(["CASH", "CHECK", "CREDIT_CARD", "DEBIT_CARD", "ACH", "WIRE_TRANSFER", "INSURANCE"]),
  ...
  try {
    ...
    const allPayments = [
      ...paymentEntries.map(entry => ({
        ...
        paymentMethod: entry.type === "INSURANCE_PAYMENT" ? "INSURANCE" : "UNKNOWN",
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const ledgerEntry = await tx.ledgerEntry.create({
        data: {
          ...
          practiceId: user.practiceId,
          type: validatedData.paymentMethod === "INSURANCE" ? "INSURANCE_PAYMENT" : "PAYMENT",

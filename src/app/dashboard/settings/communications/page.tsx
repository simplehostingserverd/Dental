...
export default function CommunicationsSettingsPage() {
	...
	const testConnection = async (platform: 'whatsapp' | 'telegram' | 'signal') => {
		...
		try {
			...
			if (isAvailable) {
				toast.success(`${platform.charAt(0).toUpperCase() + platform.slice(1)} connection successful!`);
			} else {
	...
	const updateSettings = (platform: keyof CommunicationSettings, field: string, value: string | boolean) => {
		setSettings(prev => ({
			...prev,
			[platform]: {
				...prev[platform],
	...
	const getStatusBadge = (enabled: boolean, testResult: boolean | null) => {
		...
		if (testResult) {
			return <Badge className="bg-green-500 hover:bg-green-600"><CheckCircle className="mr-1 h-3 w-3" />Connected</Badge>;
		}
	...
	return (
		<div className="container mx-auto max-w-4xl p-6">
			<div className="mb-8">
				...
				<p className="text-gray-600">Configure API keys and settings for patient communication platforms</p>
			</div>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-gray-600 text-sm">Configure WhatsApp Business API for patient messaging</p>
								</div>
								{getStatusBadge(settings.whatsapp.enabled, testResults.whatsapp)}
							</div>
						</CardHeader>
									...
									<div className="space-y-2">
										...
										<div className="relative">
											<Input
												...
												onChange={(e) => updateSettings('whatsapp', 'webhookSecret', e.target.value)}
											...
											>
												{showSecrets.whatsappSecret ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
											</Button>
										</div>
									</div>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-gray-600 text-sm">Configure Telegram Bot for patient messaging</p>
								</div>
								{getStatusBadge(settings.telegram.enabled, testResults.telegram)}
							</div>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="flex items-center space-x-2">
								...
								<>
									<div className="space-y-2">
										...
										<div className="relative">
											...
											>
												{showSecrets.telegramToken ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
											</Button>
										</div>
									</div>
									...
									<div className="space-y-2">
										...
										<div className="relative">
											<Input
												...
												onChange={(e) => updateSettings('telegram', 'webhookSecret', e.target.value)}
											...
											>
												{showSecrets.telegramSecret ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
											</Button>
										</div>
									</div>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-gray-600 text-sm">Configure Signal CLI for secure patient messaging</p>
								</div>
						...
						<CardContent className="space-y-4">
							<div className="flex items-center space-x-2">
								<input
									...
									onChange={(e) => updateSettings('signal', 'enabled', e.target.checked)}
									className="rounded"
								/>
								<Label htmlFor="signal-enabled">Enable Signal Integration</Label>
							</div>
								...
								<>
									<div className="space-y-2">
										...
										<Input
											...
											onChange={(e) => updateSettings('signal', 'phoneNumber', e.target.value)}
									...
									<div className="space-y-2">
										...
										<Input
											...
											onChange={(e) => updateSettings('signal', 'apiUrl', e.target.value)}
									...
									<div className="rounded-lg bg-blue-50 p-4">
										<div className="flex">
											...
											<div className="ml-3">
												<h3 className="font-medium text-blue-800 text-sm">Signal CLI Setup Required</h3>
												<div className="mt-2 text-blue-700 text-sm">
													<p>Signal requires Signal CLI to be installed and running. Please ensure:</p>

...
export async function checkEnvironment() {
	...
	for (const check of envChecks) {
		...
		const statusIcon =
			check.status === "OK" ? "✅" : check.status === "MISSING" ? "❌" : "⚠️";
		const maskedValue =
			check.value && check.key.toLowerCase().includes("secret")
				? `${check.value.substring(0, 8)}...`
				: check.value || "NOT SET";

		console.log(
			`${statusIcon} ${check.key.padEnd(20)} | ${check.status.padEnd(8)} | ${maskedValue}`,
		);
	...
	try {
		...
		try {
			...
			console.log(
				`✅ PracticeUser table accessible (${practiceUserCount} records)`,
			);
		} catch (error) {
		...
		try {
			...
			console.log(
				`✅ PatientUser table accessible (${patientUserCount} records)`,
			);
		} catch (error) {
		...
		if (error instanceof Error && error.message.includes("P1017")) {
			console.log(
				"   💡 This usually means the database connection was closed",
			);
			console.log(
				"   💡 Check if your database URL is correct and the database is running",
			);
		}
	} finally {
		try {
...
export async function generateMissingEnvVars() {
	...
	if (
		...
		process.env.PATIENT_JWT_SECRET === "placeholder"
	) {
		const crypto = await import("node:crypto");

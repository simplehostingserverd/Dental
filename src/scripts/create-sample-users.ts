...
const sampleUsers: SampleUser[] = [
	...
	},
	{
		...
		role: "DOCTOR",
		description:
			"Dentist - Access to patient records, treatments, and scheduling",
	},
	{
		type: "practice",
	...
	},
	{
		...
		role: "STAFF",
		description:
			"General Staff - Limited access to scheduling and basic features",
	},
	{
		type: "practice",
		...
		role: "RECEPTIONIST",
		description:
			"Receptionist - Access to scheduling, patient check-in, and billing",
	},
	// Patient Users
	{
		type: "patient",
		...
		lastName: "Smith",
		description:
			"Patient Portal User - Access to personal health records and appointments",
	},
	{
		type: "patient",
	...
	try {
		...
		for (const user of sampleUsers.filter((u) => u.type === "practice")) {
			try {
				...
				if (existingUser) {
					console.log(
						`⚠️  Practice user ${user.email} already exists - skipping`,
					);
					continue;
				}
		...
		for (const user of sampleUsers.filter((u) => u.type === "patient")) {
			try {
				...
				if (existingPatientUser) {
					console.log(
						`⚠️  Patient user ${user.email} already exists - skipping`,
					);
					continue;
				}
		...
		for (const user of sampleUsers.filter((u) => u.type === "practice")) {
			console.log(
				`${user.role?.padEnd(12)} | ${user.email.padEnd(25)} | ${user.password}`,
			);
		}
		...
		console.log(
			"2. Visit http://localhost:3000/auth/signin for practice staff",
		);
		console.log(
			"3. Visit http://localhost:3000/patient/auth/signin for patients",
		);
		console.log(
			"4. Use any of the credentials above to test the authentication",
		);
	} catch (error) {

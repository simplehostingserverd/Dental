...
export class QuickActionsService {
   ...
   */
  static async scheduleEmergencyAppointment(data: QuickActionData): Promise<QuickActionResult> {
    try {
   ...
   */
  static async processPayment(data: QuickActionData): Promise<QuickActionResult> {
    try {
      const response = await fetch('/api/dashboard/billing/payments', {
        ...
        body: JSON.stringify({
          patientId: data.patientId,
          amount: parseFloat(data.paymentAmount || '0'),
   ...
   */
  static async verifyInsurance(data: QuickActionData): Promise<QuickActionResult> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to verify insurance';
      toast.error(message);
      return {
        success: false,
        message
  ...
  static async sendForms(data: QuickActionData): Promise<QuickActionResult> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to send forms';
      toast.error(message);
      return {
        success: false,
        message
   ...
   * Print Schedule
   */
  static async printSchedule(data: QuickActionData): Promise<QuickActionResult> {
    try {
        ...
        printWindow.document.write(`
          <html>
            ...
            <body>
              ...
              ${result.appointments?.map((apt: any) => `
      ...
      const message = error instanceof Error ? error.message : 'Failed to print schedule';
      toast.error(message);
      return {
        success: false,
        message
   ...
   */
  static async startPatientIntake(data: QuickActionData): Promise<QuickActionResult> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to start patient intake';
      toast.error(message);
      return {
        success: false,
        message
   ...
   */
  static async sendAppointmentReminders(data: QuickActionData): Promise<QuickActionResult> {
    try {
   ...
   * Check In Patient
   */
  static async checkInPatient(data: QuickActionData): Promise<QuickActionResult> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to check in patient';
      toast.error(message);
      return {
        success: false,
        message
   ...
   * Make Quick Call
   */
  static async makeQuickCall(data: QuickActionData): Promise<QuickActionResult> {
    try {
      if (data.phoneNumber) {
   ...
   */
  static async executeAction(actionType: string, data: QuickActionData): Promise<QuickActionResult> {
    switch (actionType) {
      case 'emergency-appointment':
        return this.scheduleEmergencyAppointment(data);
      case 'payment-collection':
        return this.processPayment(data);
      case 'insurance-verification':
        return this.verifyInsurance(data);
      case 'send-forms':
        return this.sendForms(data);
      case 'print-schedule':
        return this.printSchedule(data);
      case 'patient-intake':
        return this.startPatientIntake(data);
      case 'send-reminders':
        return this.sendAppointmentReminders(data);
      case 'check-in':
        return this.checkInPatient(data);
      case 'quick-call':
        return this.makeQuickCall(data);
      default:
        return {
          success: false,

...
export class CrudService {
   ...
   * Generic GET request
   */
  async get<T>(endpoint: string, params?: PaginationParams): Promise<CrudResponse<T>> {
    try {
   ...
   * Bulk operations
   */
  async bulkUpdate<T>(endpoint: string, ids: string[], updates: any): Promise<CrudResponse<T>> {
    return this.update(endpoint, { ids, updates });
  }

  async bulkDelete<T>(endpoint: string, ids: string[]): Promise<CrudResponse<T>> {

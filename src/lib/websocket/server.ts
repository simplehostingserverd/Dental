...
class WebSocketManager {
	...
	private connectedUsers = new Map<
		string,
		{ socket: Socket; userId: string; practiceId: string; role: string }
	>();
	...
	broadcastAppointmentUpdate(data: AppointmentUpdate) {
		...
		if (data.appointment.provider) {
			this.io
				.to(`practice:${data.practiceId}`)
				.emit("doctor:appointment_update", {
					...data,
					timestamp: new Date(),
				});
		}
		...
		console.log(
			`Broadcasted appointment update: ${data.action} for appointment ${data.appointment.id}`,
		);
	}
	...
	broadcastPatientUpdate(data: PatientUpdate) {
		...
		console.log(
			`Broadcasted patient update: ${data.action} for patient ${data.patient.id}`,
		);
	}
	...
	broadcastTaskUpdate(data: TaskUpdate) {
		...
		console.log(
			`Broadcasted task update: ${data.action} for task ${data.task.id}`,
		);
	}
	...
	broadcastNotification(data: NotificationUpdate) {
		...
		console.log(
			`Broadcasted notification to roles: ${data.targetRoles.join(", ")}`,
		);
	}
	...
	sendToUser(userId: string, event: string, data: any) {
		...
		const userConnection = Array.from(this.connectedUsers.values()).find(
	...
	getConnectedUsersCount(practiceId: string): number {
		return Array.from(this.connectedUsers.values()).filter(
			(conn) => conn.practiceId === practiceId,
		).length;
	}

...
export class TelegramService {
  ...
  constructor() {
    ...
    if (!this.botToken) {
      console.warn('Telegram bot token not configured. Telegram features will be disabled.');
    }
  }
   ...
   * Send a text message
   */
  async sendTextMessage(chatId: string | number, message: string): Promise<TelegramResponse | null> {
    if (!this.isAvailable()) {
    ...
    const payload: TelegramMessage = {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [
            { text: '📍 Ver Ubicación', url: 'https://maps.google.com/?q=' + encodeURIComponent(clinicAddress) }
          ]
        ]
      }
    };
...
<i>Clínica Dental Cognident</i>`;
    ...
    const payload: TelegramMessage = {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: '📅 Agendar Cita', url: 'https://cognident.org/es/appointments/book' },
  ...
  private async handleMessage(message: any): Promise<void> {
    ...
    console.log(`Telegram message from ${userName} (${chatId}): ${message.text}`);
...
¡Que tengas un excelente día! 😊`
   ...
   */
  private async sendMessage(method: string, payload: any): Promise<TelegramResponse | null> {
    try {

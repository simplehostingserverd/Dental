...
export class GoogleTranslateService {
  ...
  async translate(text: string, targetLanguage: string, sourceLanguage?: string): Promise<string> {
    ...
    try {
      ...
      const translatedText = response.data.translations[0]?.translatedText || text;
   ...
   */
  private async makeTranslationRequest(request: TranslationRequest): Promise<GoogleTranslateResponse> {
    const body: any = {
      q: request.text,
...
export function useGoogleTranslate() {
  ...
  const translate = async (text: string, targetLanguage: string = 'es'): Promise<string> => {
    setIsLoading(true);
    setError(null);
  ...
  const translateBatch = async (texts: string[], targetLanguage: string = 'es'): Promise<string[]> => {
    setIsLoading(true);
    setError(null);

    try {
      const results = await googleTranslate.translateBatch(texts, targetLanguage);
      return results;
    } catch (err) {

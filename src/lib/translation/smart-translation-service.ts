...
export class SmartTranslationService {
  ...
  private static readonly MEDICAL_TERMS: Record<string, Record<string, string>> = {
    es: {
  ...
  async translate(text: string): Promise<string> {
    ...
    // Try API translation
    try {
      const apiTranslation = await this.autoTranslator.translate(text, this.options.targetLanguage);
  ...
  private getStaticTranslation(text: string): string | null {
    ...
    const staticTerms = SmartTranslationService.MEDICAL_TERMS[this.options.targetLanguage];
  ...
  private getFallbackTranslation(text: string): string | null {
    ...
    try {
      const fallback = this.autoTranslator['translateWithFallback'](text, this.options.targetLanguage);
  ...
  private cacheTranslation(text: string, translation: string, source: 'static' | 'api' | 'fallback'): void {
    const cacheKey = `${text}-${this.options.targetLanguage}`;
    this.cache[cacheKey] = {
      translation,
      timestamp: Date.now(),
  ...
  async preloadCommonTranslations(): Promise<void> {
    const commonTerms = [
      'appointment', 'patient', 'treatment', 'cleaning', 'examination',
      'consultation', 'emergency', 'follow-up', 'insurance', 'payment',
      'schedule', 'cancel', 'confirm', 'reschedule', 'reminder'
    ];

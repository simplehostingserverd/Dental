...
export class RateLimitService {
   ...
   */
  static async resetRateLimit(type: keyof typeof rateLimiters, key: string): Promise<void> {
    try {
      const limiter = rateLimiters[type]
  ...
  ): Promise<{ allowed: boolean; failedCheck?: string; msBeforeNext?: number }> {
    for (const check of checks) {
      const result = await this.checkRateLimit(check.type, check.key)
      if (!result.allowed) {
        return {
          allowed: false,
  ...
  static createMiddleware(type: keyof typeof rateLimiters) {
    return async (req: any, res: any, next: () => void) => {
      ...
      const result = await this.checkRateLimit(type, key)
      
      if (!result.allowed) {
        const timeRemaining = this.formatTimeRemaining(result.msBeforeNext || 0)
      ...
      res.setHeader('X-RateLimit-Reset', new Date(Date.now() + (result.msBeforeNext || 0)))

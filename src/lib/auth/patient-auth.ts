...
export const PatientAuthService = {
   ...
   */
  async login(credentials: PatientLoginCredentials, ipAddress: string): Promise<PatientLoginResult> {
    ...
    try {
      ...
      if (!patientUser.isActive) {
        return { success: false, error: "Account is deactivated. Contact your dental practice." }
      }
      ...
      // Verify password
      if (!patientUser.password || !(await PasswordService.verifyPassword(password, patientUser.password))) {
      ...
      if (patientUser.twoFactorEnabled) {
        ...
        if (!patientUser.twoFactorSecret || !TwoFactorService.verifyToken(twoFactorToken, patientUser.twoFactorSecret)) {
          await this.handleFailedLogin(patientUser.id)
   ...
   */
  async register(registrationData: PatientRegistrationData): Promise<{ success: boolean; user?: any; error?: string }> {
    try {
